  ...
  () => {
    ...
    test('Empty user name', async ({ identityPage, visual }) => {
      await visual.step('Then validate Login form fields labels', async () => {
        await expect
          .soft(
            identityPage.userNameLabel,
            `Expect Username label: "${i18n.identity.username}"`
          )
          .toHaveText(i18n.identity.username);
        await expect
          .soft(
            identityPage.passwordLabel,
            `Expect Password label: "${i18n.identity.password}"`
          )
          .toHaveText(i18n.identity.password);
      });
      await visual.step(
        `When User fill in credentials: "${wrongUser.username}/${wrongUser.password}"`,
        async () => {
      ...
      await visual.step(
        'Then validate failed login error message',
        async () => {
          await expect(

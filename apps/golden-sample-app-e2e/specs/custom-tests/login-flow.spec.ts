...
test.describe(
  ...
  () => {
    ...
    test('Login as user with wrong credentials', async ({ identityPage, visual }) => {
    ...
    test('[Steps] Login as user with wrong credentials', async ({ identityPage, visual }) => {
      await test.step(
        `When User has entered credentials: "${wrongUser.username} / ${wrongUser.password}"`, async () => {

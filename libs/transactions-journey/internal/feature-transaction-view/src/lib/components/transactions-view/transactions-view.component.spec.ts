...
/**
 * Test suite for S5: Empty State Handling
 * JIRA-001: View Transactions by Account
 */
describe('S5: Empty State Handling', () => {
  let transactions$$: BehaviorSubject<TransactionItem[] | undefined>;
  let arrangements$$: BehaviorSubject<ProductSummaryItem[]>;
  let fixture: ComponentFixture<TransactionsViewComponent>;
  ...
  const setup = (initialTransactions: TransactionItem[] = []) => {
    transactions$$ = new BehaviorSubject<TransactionItem[] | undefined>(initialTransactions);
    arrangements$$ = new BehaviorSubject<ProductSummaryItem[]>(mockAccounts);
      ...
      providers: [
        {
          ...
          useValue: {
            snapshot: { data: { title: 'Transactions' } },
            queryParams: of({}),
            queryParamMap: of({ get: () => 'account-1' }),
          },
        },
  ...
  it('should_not_show_empty_state_when_transactions_exist', () => {
    // Arrange
    const mockTx: TransactionItem = {
      ...debitMockTransaction,
      id: 'tx-1',
      arrangementId: 'account-1',
    };
  ...
  it('should_show_transaction_items_when_transactions_exist', () => {
    // Arrange
    const mockTx: TransactionItem = {
      ...debitMockTransaction,
      id: 'tx-1',
    ...
    // Assert
    const transactionItems = fixture.nativeElement.querySelectorAll(
      '[data-role="transactions-view__item-container"]'
    );
